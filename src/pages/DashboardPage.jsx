...
export function DashboardPage() {
  ...
  async function runRefresh() {
    setNotice("");
    try {
      await api.post("/ingestion/run", { sources: ["recife_ckan", "inmet"] });
      await query.refetch();
  ...
  return (
    <div className="page-stack">
      ...
      <section className="control-bar" aria-label="Filtros do painel">
        ...
        <Link className="icon-text-button" to={`/relatorios?module=${module}&category=${category}&municipality_code=${municipality}&period_days=${period}&window_days=${windowDays}`}>Relatórios</Link>
      ...
      <section className="three-column-grid">
        ...
        <article className="surface-card"><div className="card-header"><div><span className="section-kicker">Governança</span><h2>Modelo em produção</h2></div><span>Rastreabilidade</span></div><ModelSummary model={view.model} /></article>
      </section>
